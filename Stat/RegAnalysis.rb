...
module Stat
  ...
  class RegAnalysisEMA < RegAnalysis
    ...
    ##--------------------------------------------------
    def initialize(dimY, dimX, alpha = DefaultAlpha) ;
      setup(dimY, dimX, alpha) ;
    end

    ##--------------------------------------------------
    def setup(dimY, dimX, alpha) ;
      super(dimY, dimX) ;
      @alpha = alpha ;
    end
    ...
    ##--------------------------------------------------
    def updateByEma(current, newValue, a = @alpha)
    ...
    def pushData(y, x)
      if(@count == 0) then # first case
        ...
        (0...@dimY).each{|i|
          yVal = accessVector(y,i) ;
          @sumY[i,0] = updateByEma(@sumY[i,0], yVal) ;
          (0...@dimX).each{|j|
        ...
        (0...@dimX).each{|i|
          xVal = accessVector(x,i) ;
          @sumX[i,0] = updateByEma(@sumX[i,0], xVal) ;
          (0...@dimX).each{|j|
